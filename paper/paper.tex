...
Most existing tools run only on a single computer and are limited in terms of data size by its memory. 
The most promising solution to deal with the large-scale data is scaling systems and exploiting parallelism.
...
This language-independent representation enables high-level algebraic optimizations.
...
Two well known machine learning (ML) algorithms, namely PageRank and Gaussian non-negative matrix factorization (GNMF), were implemented with Gilbert. 
